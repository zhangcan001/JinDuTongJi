...
function validateTaskPayload(data) {
  ...
  if (!isDateField(data.plannedStart)) problems.push("计划开始日期格式不正确。");
  if (!isDateField(data.planned)) problems.push("计划日期格式不正确。");
  if (data.plannedStart && data.planned && new Date(data.plannedStart) > new Date(data.planned)) {
    problems.push("计划开始日期不能晚于计划完成日期。");
  }
