const els = {
  pageTitle: document.querySelector("#pageTitle"),
  roleSelect: document.querySelector("#roleSelect"),
  globalSearchInput: document.querySelector("#globalSearchInput"),
  contractorUnitSelect: document.querySelector("#contractorUnitSelect"),
  projectFilter: document.querySelector("#projectFilter"),
  officeModeBtn: document.querySelector("#officeModeBtn"),
  saveStatus: document.querySelector("#saveStatus"),
  toast: document.querySelector("#toast"),
  detailOverlay: document.querySelector("#detailOverlay"),
  detailOverlayTitle: document.querySelector("#detailOverlayTitle"),
  detailOverlayBody: document.querySelector("#detailOverlayBody"),
  detailOverlayClose: document.querySelector("#detailOverlayClose"),
  todayTodoList: document.querySelector("#todayTodoList"),
  todaySummary: document.querySelector("#todaySummary"),
  dashboardConfig: document.querySelector("#dashboardConfig"),
  dashboardImportMount: document.querySelector("#dashboardImportMount"),
  overallProgress: document.querySelector("#overallProgress"),
  progressTrend: document.querySelector("#progressTrend"),
  delayedCount: document.querySelector("#delayedCount"),
  dueSoonCount: document.querySelector("#dueSoonCount"),
  openIssueCount: document.querySelector("#openIssueCount"),
  warningList: document.querySelector("#warningList"),
  taskTable: document.querySelector("#taskTable"),
  taskCount: document.querySelector("#taskCount"),
  excelRecordCount: document.querySelector("#excelRecordCount"),
  excelRecordHead: document.querySelector("#excelRecordHead"),
  excelRecordTable: document.querySelector("#excelRecordTable"),
  taskBuildingSelect: document.querySelector("#taskBuildingSelect"),
  taskSystemSelect: document.querySelector("#taskSystemSelect"),
  excelInput: document.querySelector("#excelInput"),
  importResult: document.querySelector("#importResult"),
  importValidationReport: document.querySelector("#importValidationReport"),
  downloadTemplateBtn: document.querySelector("#downloadTemplateBtn"),
  pasteImportText: document.querySelector("#pasteImportText"),
  pasteImportBtn: document.querySelector("#pasteImportBtn"),
  saveBaselineBtn: document.querySelector("#saveBaselineBtn"),
  backendBackupBtn: document.querySelector("#backendBackupBtn"),
  backendExportJsonBtn: document.querySelector("#backendExportJsonBtn"),
  backendExportSqliteBtn: document.querySelector("#backendExportSqliteBtn"),
  backendImportJsonInput: document.querySelector("#backendImportJsonInput"),
  exportDelayBtn: document.querySelector("#exportDelayBtn"),
  exportTasksBtn: document.querySelector("#exportTasksBtn"),
  exportIssuesBtn: document.querySelector("#exportIssuesBtn"),
  exportReportBtn: document.querySelector("#exportReportBtn"),
  printReportBtn: document.querySelector("#printReportBtn"),
  exportImportDiffBtn: document.querySelector("#exportImportDiffBtn"),
  exportImportErrorsBtn: document.querySelector("#exportImportErrorsBtn"),
  autoIssueBtn: document.querySelector("#autoIssueBtn"),
  applyDataFixBtn: document.querySelector("#applyDataFixBtn"),
  approveImportsBtn: document.querySelector("#approveImportsBtn"),
  exportBackupBtn: document.querySelector("#exportBackupBtn"),
  undoBtn: document.querySelector("#undoBtn"),
  redoBtn: document.querySelector("#redoBtn"),
  cleanupDataBtn: document.querySelector("#cleanupDataBtn"),
  importModeSelect: document.querySelector("#importModeSelect"),
  importScopeSelect: document.querySelector("#importScopeSelect"),
  importUpdatePolicySelect: document.querySelector("#importUpdatePolicySelect"),
  importDuplicatePolicySelect: document.querySelector("#importDuplicatePolicySelect"),
  backupInput: document.querySelector("#backupInput"),
  importDiffPanel: document.querySelector("#importDiffPanel"),
  importImpactPanel: document.querySelector("#importImpactPanel"),
  importPreviewPanel: document.querySelector("#importPreviewPanel"),
  baselinePanel: document.querySelector("#baselinePanel"),
  weightPanel: document.querySelector("#weightPanel"),
  importVersionPanel: document.querySelector("#importVersionPanel"),
  restorePointPanel: document.querySelector("#restorePointPanel"),
  backendBackupPanel: document.querySelector("#backendBackupPanel"),
  dataHealthPanel: document.querySelector("#dataHealthPanel"),
  auditLogPanel: document.querySelector("#auditLogPanel"),
  auditSearchInput: document.querySelector("#auditSearchInput"),
  auditActionFilter: document.querySelector("#auditActionFilter"),
  auditRoleFilter: document.querySelector("#auditRoleFilter"),
  systemHealthPanel: document.querySelector("#systemHealthPanel"),
  systemBackupPanel: document.querySelector("#systemBackupPanel"),
  systemLogPanel: document.querySelector("#systemLogPanel"),
  systemRestorePanel: document.querySelector("#systemRestorePanel"),
  performancePanel: document.querySelector("#performancePanel"),
  refreshSystemBtn: document.querySelector("#refreshSystemBtn"),
  resolveBackendConflictBtn: document.querySelector("#resolveBackendConflictBtn"),
  runMaintenanceBtn: document.querySelector("#runMaintenanceBtn"),
  taskSearchInput: document.querySelector("#taskSearchInput"),
  taskStatusFilter: document.querySelector("#taskStatusFilter"),
  taskSmartFilter: document.querySelector("#taskSmartFilter"),
  taskBuildingFilter: document.querySelector("#taskBuildingFilter"),
  taskFloorFilter: document.querySelector("#taskFloorFilter"),
  taskOwnerFilter: document.querySelector("#taskOwnerFilter"),
  taskSortSelect: document.querySelector("#taskSortSelect"),
  taskFilterResetBtn: document.querySelector("#taskFilterResetBtn"),
  taskViewSelect: document.querySelector("#taskViewSelect"),
  saveTaskViewBtn: document.querySelector("#saveTaskViewBtn"),
  clearSavedViewsBtn: document.querySelector("#clearSavedViewsBtn"),
  taskColumnToggles: document.querySelector("#taskColumnToggles"),
  taskPagination: document.querySelector("#taskPagination"),
  selectAllTasks: document.querySelector("#selectAllTasks"),
  bulkTaskToolbar: document.querySelector("#bulkTaskToolbar"),
  bulkTaskSummary: document.querySelector("#bulkTaskSummary"),
  bulkIssueBtn: document.querySelector("#bulkIssueBtn"),
  bulkExportBtn: document.querySelector("#bulkExportBtn"),
  bulkEditOwnerBtn: document.querySelector("#bulkEditOwnerBtn"),
  bulkEditNoteBtn: document.querySelector("#bulkEditNoteBtn"),
  bulkDeleteBtn: document.querySelector("#bulkDeleteBtn"),
  issueBoard: document.querySelector("#issueBoard"),
  issueTaskSelect: document.querySelector("#issueTaskSelect"),
  issueForm: document.querySelector("#issueForm"),
  issueSubmitBtn: document.querySelector("#issueSubmitBtn"),
  exportNoticeBtn: document.querySelector("#exportNoticeBtn"),
  cancelIssueEditBtn: document.querySelector("#cancelIssueEditBtn"),
  issueAttachmentInput: document.querySelector("#issueAttachmentInput"),
  buildingGrid: document.querySelector("#buildingGrid"),
  buildingModel: document.querySelector("#buildingModel"),
  modelDetail: document.querySelector("#modelDetail"),
  modelSummary: document.querySelector("#modelSummary"),
  modelBuildingFilter: document.querySelector("#modelBuildingFilter"),
  modelUnitFilter: document.querySelector("#modelUnitFilter"),
  modelSystemFilter: document.querySelector("#modelSystemFilter"),
  modelStatusFilter: document.querySelector("#modelStatusFilter"),
  modelDataOnlyToggle: document.querySelector("#modelDataOnlyToggle"),
  modelAutoRotateBtn: document.querySelector("#modelAutoRotateBtn"),
  modelResetFilterBtn: document.querySelector("#modelResetFilterBtn"),
  modelFloorPanel: document.querySelector("#modelFloorPanel"),
  modelTooltip: document.querySelector("#modelTooltip"),
  disciplineLegend: document.querySelector("#disciplineLegend"),
  scopeUnitGrid: document.querySelector("#scopeUnitGrid"),
  dictionarySummary: document.querySelector("#dictionarySummary"),
  dictionaryGrid: document.querySelector("#dictionaryGrid"),
  scopeSummary: document.querySelector("#scopeSummary"),
  scopeManageSummary: document.querySelector("#scopeManageSummary"),
  buildingScopeForm: document.querySelector("#buildingScopeForm"),
  unitScopeForm: document.querySelector("#unitScopeForm"),
  buildingBatchForm: document.querySelector("#buildingBatchForm"),
  saveBuildingScopeBtn: document.querySelector("#saveBuildingScopeBtn"),
  cancelBuildingEditBtn: document.querySelector("#cancelBuildingEditBtn"),
  saveUnitScopeBtn: document.querySelector("#saveUnitScopeBtn"),
  cancelUnitEditBtn: document.querySelector("#cancelUnitEditBtn"),
  scopeMaintenanceList: document.querySelector("#scopeMaintenanceList"),
  projectAdminForm: document.querySelector("#projectAdminForm"),
  projectCopyFromSelect: document.querySelector("#projectCopyFromSelect"),
  projectTemplateSelect: document.querySelector("#projectTemplateSelect"),
  projectSubmitBtn: document.querySelector("#projectSubmitBtn"),
  saveProjectTemplateBtn: document.querySelector("#saveProjectTemplateBtn"),
  cancelProjectEditBtn: document.querySelector("#cancelProjectEditBtn"),
  projectAdminList: document.querySelector("#projectAdminList"),
  projectAdminSummary: document.querySelector("#projectAdminSummary"),
  taskForm: document.querySelector("#taskForm"),
  taskAttachmentInput: document.querySelector("#taskAttachmentInput"),
  taskSubmitBtn: document.querySelector("#taskSubmitBtn"),
  cancelTaskEditBtn: document.querySelector("#cancelTaskEditBtn"),
  deviationSummary: document.querySelector("#deviationSummary"),
  deviationList: document.querySelector("#deviationList"),
  dependencyList: document.querySelector("#dependencyList"),
  unitRanking: document.querySelector("#unitRanking"),
  generateWeeklyBtn: document.querySelector("#generateWeeklyBtn"),
  copyWeeklyBtn: document.querySelector("#copyWeeklyBtn"),
  carouselBtn: document.querySelector("#carouselBtn"),
  carouselExitBtn: document.querySelector("#carouselExitBtn"),
  weeklyReportOutput: document.querySelector("#weeklyReportOutput"),
  weeklySummary: document.querySelector("#weeklySummary"),
  analyticsGrid: document.querySelector("#analyticsGrid"),
  analyticsSummary: document.querySelector("#analyticsSummary"),
  progressQueryInput: document.querySelector("#progressQueryInput"),
  progressQueryUnitFilter: document.querySelector("#progressQueryUnitFilter"),
  progressQueryBuildingFilter: document.querySelector("#progressQueryBuildingFilter"),
  progressQueryFloorFilter: document.querySelector("#progressQueryFloorFilter"),
  progressQueryStatusFilter: document.querySelector("#progressQueryStatusFilter"),
  progressQueryResetBtn: document.querySelector("#progressQueryResetBtn"),
  progressQuerySummary: document.querySelector("#progressQuerySummary"),
  progressQueryStats: document.querySelector("#progressQueryStats"),
  progressQueryTable: document.querySelector("#progressQueryTable"),
  visualDashboardSummary: document.querySelector("#visualDashboardSummary"),
  floorHeatmap: document.querySelector("#floorHeatmap"),
  unitProgressChart: document.querySelector("#unitProgressChart"),
  dashboardInsightSummary: document.querySelector("#dashboardInsightSummary"),
  dashboardHealthList: document.querySelector("#dashboardHealthList"),
  dashboardImportHistory: document.querySelector("#dashboardImportHistory"),
  basementSummary: document.querySelector("#basementSummary"),
  basementCutaway: document.querySelector("#basementCutaway"),
  chart: document.querySelector("#progressChart")
};

let modelState = null;
let selectedBuildingName = "";
let selectedModelFloor = "";
let lastImportFocus = null;
let pendingImport = null;
let globalSearchQuery = "";
const selectedTaskIds = new Set();
const taskFilters = {
  query: "",
  status: "all",
  building: "all",
  floor: "all",
  owner: "all",
  smart: "all",
  sort: "plannedAsc",
  page: 1,
  pageSize: 120
};
let stateCache = { version: 0, projectItems: new Map() };
let drawModelFrame = null;
const perfMetrics = {
  lastRenderMs: 0,
  lastSaveMs: 0,
  renderCount: 0,
  lastRenderScope: "init"
};

exposeAppApi("els", els);
exposeAppApi("selectedTaskIds", selectedTaskIds);
exposeAppApi("taskFilters", taskFilters);
exposeAppApi("stateCache", stateCache);
exposeAppApi("perfMetrics", perfMetrics);
